...
# Custom TransformerEncoder wrapper to handle version compatibility
...
# Custom TransformerDecoder wrapper  
...
## MTAD_GAT Model (Comment out due to DGL dependency issue)
...
## GDN Model (Comment out due to DGL dependency issue)
# class GDN(nn.Module):
# 	def __init__(self, feats):
...
class TranAD_Basic(nn.Module):
	def __init__(self, feats):
		...
		self.transformer_encoder = CustomTransformerEncoder(encoder_layers, 1)
		decoder_layers = TransformerDecoderLayer(d_model=feats, nhead=feats, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder = CustomTransformerDecoder(decoder_layers, 1)
		self.fcn = nn.Sigmoid()
...
class TranAD_SelfConditioning(nn.Module):
	def __init__(self, feats):
		...
		self.transformer_decoder1 = CustomTransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = TransformerDecoderLayer(d_model=2 * feats, nhead=feats, dim_feedforward=16, dropout=0.1)
		self.transformer_decoder2 = CustomTransformerDecoder(decoder_layers2, 1)
		self.fcn = nn.Sequential(nn.Linear(2 * feats, feats), nn.Sigmoid())
